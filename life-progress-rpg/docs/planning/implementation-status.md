...
- 本轮计划（优先级）：
  - [ ] P1：补齐 `quality-audit-h.md` 每页核心状态核验（有/无数据、成功、失败、AI 回退、离线/恢复）。
...
- 验收证据要求（本轮）：
  - 每一项必须有命令输出或截图路径；无法执行者注明“待人工复核”。
  - 关键页面建议复核截图仍保留在 `artifacts/quality-audit/`，并在核验表标注状态。
