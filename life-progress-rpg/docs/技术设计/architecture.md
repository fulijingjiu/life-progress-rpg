...
> 状态：v0.1 本地架构已实施；云端数据库与 AI 已立项，详见[云端数据库与 AI 架构](./云端数据库与AI架构.md)
...
当前仓库只完成浏览器端 AI 请求契约与失败回退，同源 AI 代理尚未实现；正式服务端实现按[云端数据库与 AI 架构](./云端数据库与AI架构.md)推进。

v0.1 不需要 API Gateway、Redis、消息队列、PostgreSQL、多节点或微服务。后续云端阶段只增加单个 PostgreSQL/Auth/函数平台；其余组件仍需由真实容量瓶颈触发。
...
| 云端基础（已立项） | v0.1 可发布且隐私、安全方案通过评审 | Auth + PostgreSQL + RLS + 单个 AI 函数，保留 IndexedDB |
