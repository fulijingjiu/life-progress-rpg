...
1. **清单同步与分解**
   - 输入：`implementation-status.md`、`milestone-v1.md`、`quality-audit-h.md`
   - 产出：更新本轮 `implementation-status.md` 的“本轮最小切片”与“阻塞项（可控/不可控）”
   - 证据：`git diff` + 文档中的明确勾选变化
...
4. **质量门禁**
   ...
   - 产出：本轮执行命令与结果（含失败原因）
   - 证据：实际输出摘要（命令名 + 成功/失败 + 关键错误）

5. **循环更新**
   - 输入：本轮所有任务结果
   - 产出：`implementation-status.md` 更新下一轮任务
   - 证据：文档与 `git status` 一致
